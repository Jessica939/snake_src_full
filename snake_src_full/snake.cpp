...
bool Snake::hitWall()
{
    ...
    int headX = head.getX();
    int headY = head.getY();
    if (headX <= 0 || headX >= this->mEffectiveWidth - 1)
    {
        return true;
    }
    if (headY <= 0 || headY >= this->mEffectiveHeight - 1)
    {
        return true;
    }
    return false;
...
void Snake::shrink(){
    if (mSnake.size() > 0) {
        // Add your shrinking logic here, for example:
        mSnake.pop_back();
    }
}
...
SnakeBody Snake::getHead() const
{
    if (!mSnake.empty()) {
        ...
        return SnakeBody(0, 0); // Return a default position if the snake is empty
    }
}


void Snake::updateBoundary(int width, int height) {
